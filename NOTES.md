...
- **Per-env domain randomization: reset-time CPU-view writes vs. per-step GPU state — and why the "CPU" part doesn't slow training** (M4–M6). Randomizing mass/inertia (M4) and center-of-mass (M6) goes through `robot.root_physx_view.get_*/set_*`, which operate on **CPU-side buffers**. That does *not* move the simulation onto the CPU — the PhysX solver still runs on the GPU (the GB10). The distinction that matters:
  - **Per-step state** (positions, velocities, orientations, the applied wrench) lives on the **GPU** and is integrated every physics step. It never round-trips to the CPU in the hot loop.
  - **Body parameters** (mass, inertia, CoM) are *configuration*, not state. PhysX keeps them in a host buffer you write into and syncs to the GPU solver; they aren't re-integrated each step.
  - **Why it's cheap:** `_randomize_mass` / `_randomize_com` are called only in `_reset_idx`, so `set_*` fires **at most once per env per episode (~500 steps)**, on just the resetting subset. The cost is a small host→device copy of a `(num_resetting_envs, num_bodies, k)` tensor, amortized over the whole episode → negligible.
  - **Measured (this session):** M6 run 2 *with* CoM CPU-writes ran at ~0.43–0.45 s/iter — identical to M5 run 2 *without* them (~0.44 s/iter). No measurable slowdown. (Mass DR has used this same mechanism since M4 with no cost.)
  - **The one way it *would* be slow:** calling `set_coms`/`set_masses` **every physics step** would force a host↔device sync each step and hurt throughput. Randomizing only at reset avoids that — and is also physically correct: a drone's CoM/mass is a fixed per-build error for the whole flight, not something that jitters frame-to-frame.
  - **API shape gotcha:** `get_coms()` returns `(num_envs, num_bodies, 7)` = position(3) + quaternion(4); offset only the `:3` position slice. Apply the offset to a **captured default snapshot** (`self._default_coms` grabbed once in `__init__`), not the live value, or repeated resets **compound**. Mirrors Isaac Lab's `randomize_rigid_body_com` (`isaaclab/envs/mdp/events.py`); the same capture-default rule applies to `_randomize_mass` (uses `default_mass`).
