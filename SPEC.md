...
| 2026-07-12 | **M8 reward change (locked-reward exception, rule #5):** add an obstacle **proximity penalty** (grows as the nearest of the 5 Multiranger ray-distances drops below a safe margin) and **collision termination** (`died` also true when min obstacle distance < body radius). M7's waypoint reward terms (distance + vel penalties + success bonus) are unchanged. Scoped to the new `Isaac-Crazyflie-Obstacle-Direct-v0` task only. **Sensor decision:** use `MultiMeshRayCaster` (not the base `RayCaster`), because the base RayCaster bakes a single static warp mesh shared across all envs and cannot represent per-env randomized obstacles; MultiMeshRayCaster matches per-env `{ENV_REGEX_NS}/obstacle_.*` meshes with `track_mesh_transforms`. Obs 12→17 (append 5 ray distances). | Obstacle avoidance needs a collision-risk signal and a hard fail on contact; MultiMeshRayCaster is the only RayCaster-family sensor that supports per-env randomized obstacles. Waypoint reward preserved so navigation behavior carries over. | Daron |
| 2026-07-12 | **M7 reward change (locked-reward exception, rule #5):** add a discrete **success bonus** (+10.0, granted once when the drone first enters `success_radius`=0.15 m of the current waypoint, not ×dt); the three M3 terms (distance `1−tanh(d/0.8)`×15, lin_vel×−0.05, ang_vel×−0.01) are **unchanged**, now measured to the current resampling waypoint. Scoped to the new `Isaac-Crazyflie-Waypoint-Direct-v0` task only — the M3 hover reward/task is untouched. Accompanying (non-reward) changes: 4 m×4 m×3 m goal workspace (xy U(−2,2), z U(0.5,3.0)), death ceiling z>2.0→3.5 m, episode 10 s→20 s. | Waypoint following needs credit for *reaching/completing* waypoints, not just proximity; the discrete bonus + resample aligns training with the ≥90%-success bar while preserving the underlying hover behavior. | Daron |
